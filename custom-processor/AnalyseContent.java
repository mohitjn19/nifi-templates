...
@SideEffectFree
@SupportsBatching
@Tags({"analyse", "count", "text", "line", "word", "character"})
@InputRequirement(InputRequirement.Requirement.INPUT_REQUIRED)
@CapabilityDescription("Analyse various metrics on incoming text. The requested results will be recorded as attributes. "
        + "The resulting flowfile will not have its content modified.")
@WritesAttributes({
        @WritesAttribute(attribute = "analysed.line.count", description = "The number of lines of text present in the FlowFile content"),
        @WritesAttribute(attribute = "analysed.line.nonempty.count", description = "The number of lines of text (with at least one non-whitespace character) present in the original FlowFile"),
        @WritesAttribute(attribute = "analysed.word.count", description = "The number of words present in the original FlowFile"),
        @WritesAttribute(attribute = "analysed.character.count", description = "The number of characters (given the specified character encoding) present in the original FlowFile"),
})
public class AnalyseContent extends AbstractProcessor {
    ...
    // Attribute keys
    public static final String TEXT_LINE_COUNT = "analysed.line.count";
    public static final String TEXT_LINE_NONEMPTY_COUNT = "analysed.line.nonempty.count";
    public static final String TEXT_WORD_COUNT = "analysed.word.count";
    public static final String TEXT_CHARACTER_COUNT = "analysed.character.count";
    ...
    int countWordsInLine(String line, boolean splitWordsOnSymbols) throws IOException {
        if (line == null || line.trim().length() == 0) {
            ...
            if (getLogger().isDebugEnabled()) {
                ...
                getLogger().debug("Split [" + line + "] to [" + String.join(",",words) + "] (" + words.size() + ")");
